...
def transcribe_and_detect(sample):
    ...
    # generating transcript and enforcing one language (for now)
    result = pipe(sample, generate_kwargs={"task": "transcribe", "language": "<|en|>"})
    print(result["text"])
...
sample1 = Constants.FLAC_URL1
transcribe_and_detect(sample1)


# def lang_whandler(result):
#     match lang:
#         case 'en':
...
# transResult = pipe(sample1)
# lang_whandler(transResult)
